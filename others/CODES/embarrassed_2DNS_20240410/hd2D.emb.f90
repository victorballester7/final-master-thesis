...
! Some auxiliary matrixes
   ...
   ! my variables
   INTEGER :: threshold, seed, iflow,myseed,p
...
!     dir_data : global directory for I/O
   ...
   CALL MPI_BCAST(  hnu,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr) !11
   CALL MPI_BCAST( prm1,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr) !12
   CALL MPI_BCAST( prm2,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr) !13
   CALL MPI_BCAST( seed,1,MPI_INTEGER,         0,MPI_COMM_WORLD,ierr) !14
   CALL MPI_BCAST(iflow,1,MPI_INTEGER,         0,MPI_COMM_WORLD,ierr) !15
   CALL MPI_BCAST( ldir,100,MPI_CHARACTER,     0,MPI_COMM_WORLD,ierr) !16
   myseed=seed+myrank
...
!!!!!!!  RANDOM FORCING  !!!!!!!!!!!
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      CALL forcing(iflow,f0,kup,kdn,myseed,fk)  !! set fk=0
...
! mhdcheck subroutine for details.
      ...
      IF (timec.eq.cstep) THEN
         timec = 0
         CALL hdcheck(ps,fk,time,ener,enst,node)
...
! with the power spectrum
      ...
      IF (times.eq.sstep) THEN
         ...
         u = char(ju)
         ext = c // d // u
         IF (myrank.eq.0) print*,"DBG spec1"
         CALL spectrum(ps,ext,1,node)
         IF (myrank.eq.0) print*,"DBG spec2"
         CALL EnergyEnstropy_profiles(ps,p,ext,node,dir)
         IF (myrank.eq.0) print*,"DBG spec3"
         CALL laplak2(ps,C1)     ! make W
         CALL ring_vorticity(C1,ext,node)
         IF (myrank.eq.0) print*,"DBG spec4"
         CALL vectrans(ps,ps,C1,'euu',ext,node)
         IF (myrank.eq.0) print*,"DBG spec5"
         CALL vectrans(C1,ps,C1,'vrt',ext,node)
         IF (myrank.eq.0) print*,"DBG spec6"
      ENDIF
