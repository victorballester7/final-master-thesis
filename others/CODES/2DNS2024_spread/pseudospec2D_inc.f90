...
! !
   ...
   DO i = 1,r_max
      IF (Num(i).gt.0) THEN
         E_R(i) = E_R(i)/dble(Num(i))/dble(n)**(2*p) ! n^2p = (n^2)^p, where the n^2 is the normalization factor for the FFT and the other ^p is because the field is raised to the power of p
         W_R(i) = W_R(i)/dble(Num(i))/dble(n)**(2*p)
      ENDIF
   END DO

   OPEN(1,file=trim(dir)//'/EnergyProf/Energy.' // trim(p_str) //'.' // ext // '.txt')
   WRITE(1,20) E_R
20 FORMAT( E23.15 )
   CLOSE(1)
   OPEN(1,file=trim(dir)//'/EnstrophyProf/Enstrophy.'// trim(p_str) // '.' // ext // '.txt')
   WRITE(1,24) W_R
24 FORMAT( E23.15 )
   CLOSE(1)
   RETURN
