...
!*****************************************************************
SUBROUTINE initialcond(iflow,u0,kup,kdn,seed,a)
   !-----------------------------------------------------------------
...
! a     : streamfunction
   ...
   IMPLICIT NONE
   DOUBLE COMPLEX, DIMENSION(n/2+1,n)     :: a
   INTEGER :: seed, iflow, i,j
   DOUBLE PRECISION :: u0,kup,kdn,tmp1,enerk

   CALL forcing(iflow,u0,kup,kdn,seed,a)
...
!    dir: directory where the results are stored
   ...
   DOUBLE PRECISION, DIMENSION(n,n)    :: r1,r2, r3
   INTEGER     :: r,p
   INTEGER, DIMENSION(r_max) :: Num  ! Number of points in each circle
   INTEGER     :: i,j
   CHARACTER*3 :: ext
   CHARACTER*100 :: dir
   DOUBLE PRECISION :: tmp
...
20    FORMAT(8E22.14)
   ...
   DO r = 1,r_max
      WRITE(1,20) (W_R((r-1)*p_max+p),p=1,p_max)
   END DO
   CLOSE(1)

   !  DEALLOCATE(E_R,W_R,Num)
