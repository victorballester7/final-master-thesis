...
int main(void) {
  ...
  // allocate memory
  double *X =
      new double[N]; // vector of positions (x1,y1,C1,x2,y2,C2,...,xn,yn,Cn)
  double dist;
  ...
  for (int i = 0; i < n; i++) {
    ...
    r = sqrt(R * abs((double)rand() /
                     RAND_MAX)); // random radius. We do not do it uniformly to
                                 // avoid having more points per area in the
                                 // smaller subradii. With the sqrt we have more
                                 // points in the outer layers.
